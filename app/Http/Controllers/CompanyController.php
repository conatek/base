...
class CompanyController extends Controller
{
    ...
    public function getAgeRangesData($company_id)
    {
        ...
        $data = Collaborator::select(
            DB::raw('
                CASE
                    ...
                    WHEN TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) BETWEEN 25 AND 33 THEN "Entre 25 y 33"
        ...
        // Construir arrays en el orden definido, con 0 para rangos sin datos
        $labels = [];
        $values = [];
        
        foreach ($order as $range) {
            $labels[] = $range;
            $values[] = isset($data[$range]) ? $data[$range]->count : 0;
        }
        
        $age_ranges_data['labels'] = $labels;
        $age_ranges_data['values'] = $values;
    ...
    public function getAcademicLevelData($company_id)
    {
        ...
        $data = Collaborator::join('companies as cp', 'cp.id', '=', 'collaborators.company_id')
            ...
            ->where('cp.id', $company_id)
            ->select(
                'lb.level',  // Agregamos el level para poder ordenar
                DB::raw("CASE
            ...
            ->groupBy('lb.level', 'type')  // Agregamos lb.level al groupBy
            ->orderBy('lb.level', 'asc')   // Ordenamos por level ascendente
            ->get();
    ...
    public function getSocialStrataData($company_id)
    {
        ...
        $data = DB::table('collaborators')
            ...
            ->orderBy('collaborators.stratum_type_id', 'asc')  // Cambiado aquí
            ->get();
        ...
        $data = Collaborator::select(
                DB::raw("
                    CASE
                        ...
                        WHEN TIMESTAMPDIFF(MONTH, collaborator_contracts.contract_start_date, CURDATE()) BETWEEN 24 AND 71 THEN 'Entre 2 y 5'
                        WHEN TIMESTAMPDIFF(MONTH, collaborator_contracts.contract_start_date, CURDATE()) BETWEEN 72 AND 131 THEN 'Entre 6 y 10'
                        ELSE 'Mayor de 10'
                    END AS time
                "),
                DB::raw("
                    CASE
                        WHEN TIMESTAMPDIFF(MONTH, collaborator_contracts.contract_start_date, CURDATE()) < 24 THEN 1
                        WHEN TIMESTAMPDIFF(MONTH, collaborator_contracts.contract_start_date, CURDATE()) BETWEEN 24 AND 71 THEN 2
                        WHEN TIMESTAMPDIFF(MONTH, collaborator_contracts.contract_start_date, CURDATE()) BETWEEN 72 AND 131 THEN 3
                        ELSE 4
                    END AS sort_order
                "),
    ...
    public function getNextBirthdays($company_id)
    {
        ...
        $data = Collaborator::select(
            DB::raw("CONCAT_WS(' ', name, first_surname, second_surname) AS name"),
    ...
    public function getExpiringContracts($company_id)
    {
        ...
        $data = Collaborator::select(
            DB::raw("CONCAT_WS(' ', collaborators.name, collaborators.first_surname, collaborators.second_surname) AS name"),
