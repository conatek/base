    ...
    <div class="login-wrapper">
        ...
        <div class="col-lg-7 col-xl-8 login-form-side">
            <div class="login-card">
                ...
                <h1 class="welcome-text">¿Olvidaste tu contraseña?</h1>
                <p class="subtitle-text">Ingresa tu correo y te enviaremos un enlace de recuperación.</p>
                ...
                <form v-else @submit.prevent="handleSubmit">
                    ...
                    <div class="text-center">
                        <router-link to="/login" class="btn-link-custom">
...
export default {
    ...
    methods: {
        async handleSubmit() {
            ...
            try {
                const response = await window.axios.post('/forgot-password', this.form);
                this.successMessage = response.data.status || 'Se ha enviado el enlace de recuperación a tu correo.';
            } catch (error) {
                if (error.response && error.response.status === 422) {
                    this.errors = error.response.data.errors || {};
                } else {
                    this.generalError = 'Error al enviar el correo. Intenta de nuevo.';
                }
            } finally {
                this.loading = false;
            }
...
.brand-bg-image {
    ...
    background-image: url('/images/sidebar/mh-people-lineart.jpg');
...
.form-control:focus {
    ...
    border-color: #12b338;
    box-shadow: 0 0 0 4px rgba(18, 179, 56, 0.1);
}
...
.btn-link-custom { color: #12b338; text-decoration: none; font-weight: 500; font-size: 0.9rem; }
.btn-link-custom:hover { color: #ff4600; }

@media (max-width: 991px) {
    .login-wrapper { flex-direction: column; }
    .login-brand-side { min-height: 200px; padding: 2rem; flex: 0 0 auto; }
}
</style>
